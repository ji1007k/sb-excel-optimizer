...
public class PagingExcelStrategy implements ExcelDownloadStrategy {
    ...
    public void process(DownloadRequest request, ExcelContext context) {
        log.info("Processing with ROWNUM PAGING method: {}", request.getRequestId());
        ...
        try (SXSSFWorkbook workbook = excelBuilder.createSXSSFWorkbook(10)) {
            ...
            for (long startRow = 1; startRow <= totalCount; startRow += BATCH_SIZE) {
                ...
                // 페이지 처리 완료 로그
                log.debug("Processed ROWNUM page: {}-{} ({} total rows)", startRow, endRow, processedCount);
            ...
            log.error("ROWNUM Paging excel file creation failed: {}", request.getRequestId(), e);
            throw new RuntimeException("ROWNUM Paging 엑셀 파일 생성 실패: " + e.getMessage(), e);
        }
    }
    
    @Override
    public DownloadRequest.DownloadType getSupportedType() {
        return DownloadRequest.DownloadType.PAGING;
    }
}
