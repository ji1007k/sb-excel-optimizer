 ...
 * - 10만건 테스트 시 SXSSF_OFFSET_PAGING 방식에서 쿼리 오버헤드로 인해 oldway가 더 빠를 수 있음.
 * - 단, OLDWAY 다중 요청으로 OOM 발생 시 FULL GC(STW)로 인한 소요시간 + 데이터 손실을 생각하면 SXSSF_OFFSET_PAGING 방식이 안정성 측면에서 더 좋음
 */
@SpringBootTest
@ActiveProfiles("test")
@Slf4j
class SimplePerformanceTest {
    ...
    @Test
    void oldway_1회실행() {
        log.info("=== XSSF_FULL_LOAD 테스트 ===");
        
        long start = System.currentTimeMillis();
        String result = excelDownloadService.processXssfFullLoadDirectly("test", UUID.randomUUID().toString());
        long time = System.currentTimeMillis() - start;
//        30521ms
        log.info("...XSSF_FULL_LOAD 결과: {}ms", time);
    }
    ...
    void oldway_N회연속() {
        ...
        log.info("=== XSSF_FULL_LOAD {}회 연속 실행 ===", iterationCount);
        ...
        for (int i = 1; i <= iterationCount; i++) {
            ...
            CompletableFuture<String> future = CompletableFuture.supplyAsync(() -> {
                try {
                    long start = System.currentTimeMillis();
                    String result = excelDownloadService.processXssfFullLoadDirectly("test", "test-" + finalI);
                    long time = System.currentTimeMillis() - start;
...
//...{4}회차: {17568}ms - 성공
//...{5}회차: {17457}ms - 성공
        log.info("=== XSSF_FULL_LOAD {}회 완료 ===", iterationCount);
...
//        19095ms.
    ...
    @Test
    void paging_1회실행() {
        log.info("=== SXSSF_OFFSET_PAGING 테스트 ===");

        long start = System.currentTimeMillis();
        String result = excelDownloadService.processSxssfOffsetPagingDirectly("test", UUID.randomUUID().toString());
        long time = System.currentTimeMillis() - start;
//        21302ms
        log.info("...SXSSF_OFFSET_PAGING 결과: {}ms", time);
    }
    ...
    void paging_N회연속() {
        ...
        log.info("=== SXSSF_OFFSET_PAGING {}회 연속 실행 ===", iterationCount);
        ...
        for (int i = 1; i <= iterationCount; i++) {
            ...
            CompletableFuture<String> future = CompletableFuture.supplyAsync(() -> {
                long start = System.currentTimeMillis();
                String result = excelDownloadService.processSxssfOffsetPagingDirectly("test", "test-" + finalI);
                long time = System.currentTimeMillis() - start;
...
//...{4}회차: {22958}ms - 성공
//...{5}회차: {19388}ms - 성공
        log.info("=== SXSSF_OFFSET_PAGING {}회 완료 ===", iterationCount);
        log.info(sb.toString());
        log.info("...평균 소요 시간: {}ms", timeAvg.get() / iterationCount);
//        20152ms
    }
    ...
    void streaming_N회연속() {
        ...
        log.info("=== SXSSF_CURSOR_PAGING {}회 실행 ===", iterationCount);
...
//        1207ms ~ 3258ms
        log.info("...SXSSF_CURSOR_PAGING {}회 완료", iterationCount);
